...
class Outliers:
    ...
    def handle(self, df,target_column):
        ...
        if self.outliers:
            ...
            if self.outliers in ['auto', 'winz']:  
                df = Outliers._winsorization(self, df,target_column)
    ...
    def _winsorization(self, df, target_column):
        # function for outlier winsorization
        cols_num = df.select_dtypes(include=np.number).columns  # Get numerical columns
        for feature in cols_num:
            if feature != target_column:  # Skip target column
                counter = 0
                
                # Compute outlier bounds
                lower_bound, upper_bound = Outliers._compute_bounds(self, df, feature)
                
                # Ensure dtype compatibility by converting column to float if bounds are floats
                if not all(isinstance(val, int) for val in [lower_bound, upper_bound]):
                    df[feature] = df[feature].astype(float)
                
                # Winsorize the values
                for row_index, row_val in enumerate(df[feature]):
                    if row_val < lower_bound or row_val > upper_bound:
                        df.loc[row_index, feature] = lower_bound if row_val < lower_bound else upper_bound
                        counter += 1
    ...
    def _delete(self, df, target_column):
        """
        Function for deleting outliers in the data.
        """
        cols_num = df.select_dtypes(include=np.number).columns  # Select numeric columns
        for feature in cols_num:
            if feature != target_column:  # Skip the target column
                ...
                # Identify and drop rows containing outliers
                outlier_indices = df[(df[feature] < lower_bound) | (df[feature] > upper_bound)].index
                counter += len(outlier_indices)
                df = df.drop(index=outlier_indices).reset_index(drop=True)  # Drop and reset index
                
                if counter > 0:
                    logger.debug(
                        'Deletion of {} outliers succeeded for feature "{}"',
                        counter,
                        feature
                    )
        return df
